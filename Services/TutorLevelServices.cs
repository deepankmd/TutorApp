...
namespace TutorAppAPI.Services
{
    public class TutorLevelServices
    {
        ...
        public async Task<TutorLevel> GetAsync(string id) => await _tutorLevels.Find(t => t._id.ToString() == id).FirstOrDefaultAsync();

        public async Task CreateAsync(TutorLevel tutorLevel) => await _tutorLevels.InsertOneAsync(tutorLevel);

        public async Task UpdateAsync(string id, TutorLevel tutorLevel) => await _tutorLevels.ReplaceOneAsync(t => t._id.ToString() == id, tutorLevel);

        public async Task DeleteAsync(string id) => await _tutorLevels.DeleteOneAsync(t => t._id.ToString() == id);
